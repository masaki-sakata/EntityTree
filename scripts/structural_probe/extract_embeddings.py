def split_taxonomy_tree(tree_structure, node_classification, num_splits=None, random_seed=42, is_verbose=False):
    """
    ...
    Args:
        ...
        is_verbose: enable debug output
    ...
    if is_verbose:
        print(f"\n[DEBUG] Category-entity mapping for splitting:")
        for cat_qid, entities in category_entities.items():
            cat_title = tree_structure['nodes'][cat_qid]['title'] if cat_qid in tree_structure['nodes'] else cat_qid
            print(f"  {cat_title} ({cat_qid}): {len(entities)} entities")
            if len(entities) == 0:
                print(f"    WARNING: No entities for this category!")
    
    # Filter out categories with no entities
    valid_categories = [cat for cat in category_entities.keys() if len(category_entities[cat]) > 0]
    
    if len(valid_categories) == 0:
        print("ERROR: No categories with entities found. Cannot split tree.")
        return []
    
    # Get all possible combinations
    all_combinations = []
    
    if all(len(category_entities[cat]) > 0 for cat in valid_categories):
        # Generate combinations only for valid categories
        entity_lists = [category_entities[cat] for cat in valid_categories]
        ...
        if total_combinations <= 10000 and num_splits is None:
            ...
            # Sample random combinations for large trees
            if num_splits is None:
                num_splits = min(20, total_combinations)
            
            all_combinations = []
            for _ in range(num_splits):
                combination = tuple(random.choice(entity_list) for entity_list in entity_lists)
