#!/usr/bin/env python
# =============================================================================
# eval_tree_red_dim.py - Tree Distance Evaluation (JRF-compliant) with Dimensionality Reduction
# =============================================================================
"""
Evaluate predicted trees against gold trees using *true* Jaccard-Robinson-Foulds
distance (Robinson & Foulds 1981, Pompei et al. 2012) with optional PCA/LDA dimensionality reduction.
"""
...
# --------------------------------------------------------------------------- #
# NEW: Dimensionality reduction utilities
# --------------------------------------------------------------------------- #
def get_lda_model_path(model_type: str, method: str, template: str, 
                       layer: int, n_components: int) -> str:
    """Generate LDA model path based on model and parameters."""
    base_dir = "/home/masaki/hierarchical-repr/EntityTree/models/lda"
    
    # Filename uses abbreviated model name
    if model_type == "meta-llama/Meta-Llama-3-8B":
        model_filename = "Meta-Llama-3-8B"
    else:
        model_filename = model_type  # gpt2, fasttext
    
    # Construct path
    path = f"{base_dir}/{model_type}/comp_{n_components}/lda_{model_filename}_{method}_{template}_layer{layer}.joblib"
    return path
...
def apply_dimensionality_reduction(
    ...
    model_type : str
        Model name for LDA path
    method : str
        Embedding method for LDA path
    template : str
        Template name for LDA path
    layer : int
        Layer index for LDA path
    ...
    print(f"\nApplying dimensionality reduction: {reduction_type}")
    print(f"  Before: {embs.shape}")
    
    if reduction_type == "pca":
        pca = PCA(n_components=n_components)
        embs_reduced = pca.fit_transform(embs)
        print(f"  After:  {embs_reduced.shape}")
        print(f"  Explained variance ratio: {pca.explained_variance_ratio_.sum():.4f}")
        
    elif reduction_type == "lda":
        lda_path = get_lda_model_path(model_type, method, template, layer, n_components)
        print(f"  Loading LDA model from: {lda_path}")
        
        if not Path(lda_path).exists():
            raise FileNotFoundError(f"LDA model not found: {lda_path}")
        
        bundle = joblib.load(lda_path)
        lda = bundle["lda"]
        # Note: label_encoder is available but not needed for transform
        
        embs_reduced = lda.transform(embs)
        print(f"  After:  {embs_reduced.shape}")
...
# --------------------------------------------------------------------------- #
# 4. Predicted tree builder (modified to support dimensionality reduction)
# --------------------------------------------------------------------------- #
def build_predicted_tree(
    ...
    or gold tree binary conversion, with optional dimensionality reduction.
    ...
    method : str
        Embedding method
    layer : int or str
        Layer number for transformer models or "all"
    device : str
    ...
    reduction_type : str or None
        'pca', 'lda', or None for no reduction
    n_components : int
    ...
    birth_time : Dict[int, float]
        Node birth times
    """
    # Check if dimensionality reduction is applicable
    reduction_applicable_models = ["gpt2", "meta-llama/Meta-Llama-3-8B", "fasttext"]
    ...
    if model_type == "random_emb":
        if reduction_type is not None:
            print(f"Warning: Dimensionality reduction not supported for random_emb model. Skipping.")
        ...
        if reduction_type is not None and model_type not in reduction_applicable_models:
            print(f"Warning: Dimensionality reduction not supported for {model_type} model. Skipping.")
            reduction_type = None
        ...
        if isinstance(layer, str) and layer == "all":
            ...
            for li in range(embs.shape[0]):
                ...
                if reduction_type is not None:
                    layer_embs = apply_dimensionality_reduction(
                        layer_embs, reduction_type, n_components,
                        model_type, method, template_name, li
                    )
                
                # Build hierarchy from the (possibly reduced) embeddings
                hierarchy = HierarchyNode(layer_embs)
                hierarchy.calculate_persistence()
            ...
            # Apply dimensionality reduction if specified
            if reduction_type is not None:
                embs = apply_dimensionality_reduction(
                    embs, reduction_type, n_components,
                    model_type, method, template_name, int(layer)
                )
...
def main() -> None:
    ...
    # Add dimensionality reduction arguments (mutually exclusive)
    dim_group = ap.add_mutually_exclusive_group()
    dim_group.add_argument("--pca", action="store_true",
                          help="Apply PCA dimensionality reduction")
    dim_group.add_argument("--lda", action="store_true",
                          help="Apply LDA dimensionality reduction using pre-trained model")
    ap.add_argument("--n_components", type=int, default=50,
    ...
    if args.model in ["gold_binary_left", "gold_binary_balanced"]:
        ...
        pred = build_predicted_tree(
            ...
            reduction_type=reduction_type, n_components=args.n_components)
    ...
    def eval_and_save_one(layer_idx: int,
        ...
        if reduction_type:
            print(f"Dimensionality reduction: {reduction_type} (n_components={args.n_components})")
        print("-----------------------------------------------------------")
        ...
        results = dict(dataset=str(args.input), model=args.model, layer=layer_idx,
                       ...
                       dimensionality_reduction=reduction_type,
                       n_components=args.n_components if reduction_type else None,
    ...
    # Modify output directory based on dimensionality reduction
    out_dir = Path(args.output_dir)
    if reduction_type == "pca":
        out_dir = out_dir / "PCA" / f"comp_{args.n_components}"
    elif reduction_type == "lda":
        out_dir = out_dir / "LDA" / f"comp_{args.n_components}"
